...
CREATE POLICY "Admins can update demos" ON storage.objects
FOR UPDATE TO authenticated
USING (bucket_id = 'demos' AND public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can delete demos" ON storage.objects
FOR DELETE TO authenticated
USING (bucket_id = 'demos' AND public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Anyone can read demos" ON storage.objects
